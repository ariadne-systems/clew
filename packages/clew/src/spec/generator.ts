 ...
 * recognize its own output when it reads the generated set back — telling
 * tool-emitted files apart from hand-written ones.
 */
...
export const generatedHeader: (
  ...
  (generatorType: string, specSet?: string): string[] => {
    const lines = [
      ...
      " *",
      ` * @clew homepage: ${HOMEPAGE}`,
      " * @clew connector-type: req-as-code",
      ` * @clew generator-type: ${generatorType}`,
    ];
    if (specSet !== undefined) {
      lines.push(` * @clew spec-set: ${specSet}`);
    }
    lines.push(" */");
    return lines;
  },
