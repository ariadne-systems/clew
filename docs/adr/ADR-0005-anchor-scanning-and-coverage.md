...
A companion server in the broader system already works this way — a scanner produces a code-locations index and a coverage result that downstream traceback and verification consume — so the shape is proven prior art.
...
clew and a companion server in the broader system operate on the same repository and the same `.ariadnerc.json`.
...
The companion server, having no released customer, converges onto clew's vocabulary — `realizes` / `verifies`, lenses, the marker grammar — rather than the reverse.

Rationale.
The config file and the JSON artifacts are the seam at which the open tool and a companion pipeline interoperate; aligning them lets a project configured for one work with the other, and lets the two be swapped on input and output with no translation layer.
Converging the names now, before the companion has a customer, is cheap and removes a permanent dialect split over one repository.
...
A companion server converges onto clew's vocabulary and shares its configuration and output shapes, so the open tool and that pipeline interoperate over a single repository.
