...
The goal is tooling that makes the dogfooded SRDD approach reusable beyond the codebase it grew out of.
...
That central authority is the point at which a server becomes technically required.
The local variant therefore uses opaque-suffix IDs and a local index, which cannot collide silently.
