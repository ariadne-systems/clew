...
The sequential strategy opens a StateStore session, allocates the next `count` numbers for the prefix from the `sequences` section of ClewState — creating the prefix at 1 on first use and advancing its high-water mark — and saves before returning anything.
Only after the save succeeds does it format each number as `<PREFIX>-<zero-padded number>`, with the padding width read from `.clewrc.json` (for example `"idGeneration": { "mode": "sequential", "padding": 3 }`).
...
- The numbers come from the `sequences` section of ClewState and are never reused.
- The padding width is read from `.clewrc.json`; changing it changes the rendering; no width is hard-coded.
...
**Out of scope**
- The `clew mint` CLI command (its own story).
...
- [CON-003 — The numeric padding is configured in .clewrc.json](../derived-specs/CON-003-padding-configured.md)
