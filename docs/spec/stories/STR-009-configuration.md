...
**Problem / Context**
`config.ts` reads `idGeneration` (and an `idToken.pattern`) from `.clewrc.json` with defaults, but no spec or entity describes the file, so the contract is implicit.
...
**Solution Approach**
Introduce the configuration as a domain entity (ENT-002, Configuration) and a software spec (SW-009) that defines how it is read: a single `.clewrc.json` at the project root, every value defaulting when absent, sections independent.
The configuration carries three sections: `idGeneration` (mode, padding — existing), `lenses` (the spec kinds, each `{ id, description }`, with a default set), and `layout` (where each kind of artifact lives, including the story and entity prefixes).
...
Defaults are the opinionated template (ADR-0001 D8), so a project with no `.clewrc.json` behaves exactly as today.
Complete the rename ADR-0003 calls for in the same change: `Type` becomes `Lens` in every spec, and `spec.schema.json` and `story.schema.json` drop their frozen prefix and target enums to validate against the configured lenses — leaving no second source of truth and no half-migrated corpus.

**Affected specs and artifacts**
- `ENT-002` (Configuration entity — `idGeneration` + `lenses` + `layout`; in `domain-model.md`).
- `SW-009` (the tool reads project configuration from `.clewrc.json`).
...
- Field rename — every spec (ARCH-001..002, CON-001..009, NF-001..002, SW-001..009): `**Type**:` → `**Lens**:`.
- `docs/spec/schemas/spec.schema.json`: rename the `Type` field to `Lens`; replace the static `id.components.prefix` and `Lens` enums with validation against the configured lens set.
...
**Acceptance Criteria**
- Configuration is read from a single `.clewrc.json` at the project root, with per-value defaults and independent sections; with no file, every value resolves to its default.
...
- Every spec uses the `Lens` field; no spec retains `Type`.
...
- [SW-009 — The tool reads project configuration from `.clewrc.json`](../specs/SW-009-read-configuration.md)
- [CON-005 — A minted id's prefix must match the configured id token pattern](../specs/CON-005-id-prefix-must-be-configured.md) (rewritten: a minted id's prefix must be a configured prefix)
