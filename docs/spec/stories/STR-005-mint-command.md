**Title**
Add the `ariadne mint` command that allocates ids
...
The command is `ariadne mint <type> [count]`, with `count` defaulting to 1.
...
Type validation is a core id rule, not a command concern, so it lives in `mint()`: before allocating, core validates the requested type against the id token pattern read from `.ariadnerc.json` and fails fast on a malformed prefix, allocating nothing (CON-005).
...
**Acceptance Criteria**
- `ariadne mint SW` mints one id and prints it to stdout.
- `ariadne mint SW 3` prints three ids, one per line, in allocation order.
...
- The validated pattern is read from `.ariadnerc.json`; no pattern is hard-coded in the minting code.
...
- [SW-003 — The mint command emits ids to stdout, one per line](../derived-specs/SW-003-mint-command-output.md)
- [CON-005 — A minted id's prefix must match the configured id token pattern](../derived-specs/CON-005-id-matches-configured-pattern.md)
