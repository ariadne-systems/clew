...
Letting a project declare its spec sets — each by a matcher over the spec id — lets it shape the generated enums to how it actually thinks about its specs (a security set, a public-API set, …), the way a dedicated query per set does in an issue tracker.
...
**Solution Approach**
Add a `specSets` configuration section: an ordered list of `{ name, pattern }`, where `pattern` is a regular expression over the spec id.
Core groups each scanned traceable into the set whose pattern matches it; the generator then emits one symbol set per spec set (the skeleton already does this).
...
**Acceptance Criteria**
- A project can declare `specSets` — named sets, each matched by a regex over the spec id — and generation emits one symbol set per declared set.
...
**Out of scope**
- Grouping by a spec body attribute rather than the id — a richer matcher for later; the id regex is the first shot.
