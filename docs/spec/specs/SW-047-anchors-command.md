...
The output is a machine contract for the context agent, not a human report: alongside the per-id sites it carries a freshness object — `stale`, the `changedFiles` since the scan, and `head` (the commit the index reflects) — computed by asking version control which source files changed since the index was written; without version control, `stale` is reported as unknown (`null`).
...
The output carries a freshness object reporting `stale`, the `changedFiles`, and `head` (the commit the index reflects), computed from version control; without version control, `stale` is `null` (unknown).
No index or no configuration yields a structured error object and a non-zero exit.
...
- **2026-07-27** — Set active: implementation of STR-036 began.
- **2026-07-27** — The freshness object reports `head` (the commit the index reflects) rather than a wall-clock `scannedAt`, and `stale` is `null` when there is no version control.
A timestamp in the index would make it non-deterministic and break the incremental-equals-full guarantee (CON-035), and without version control there is no trustworthy age to report — so freshness is a git-derived fact or explicitly unknown.
