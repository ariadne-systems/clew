...
**Description**
If `.clewrc.json` already exists, `setup` makes no change to it and reports that a configuration is present.
...
Given an existing `.clewrc.json` — including one a project has customized — setup leaves the file byte-for-byte unchanged and reports that a configuration already exists.
