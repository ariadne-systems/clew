...
The equivalence is over the files clew scans that version control also sees: a source file clew scans but version control **ignores** is outside the change set and undetectable as such, so keeping its anchors current needs a full scan (`--full`).
...
- **2026-07-27** — Scoped the equivalence to files version control sees: a git-ignored source file clew scans is outside the change set and undetectable, so keeping its anchors current needs `--full`.
A review surfaced that clew's scan universe (config-defined) can exceed version control's (which honours `.gitignore`).
