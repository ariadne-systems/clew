...
**Description**
The tool exposes a `config` command, invoked as `clew config`, taking no positional arguments and accepting a `--json` flag.
...
`clew config` is a registered subcommand and runs with no positional arguments.
