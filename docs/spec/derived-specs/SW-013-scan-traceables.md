...
**Description**
Scanning reads the project's configured artifacts and produces the set of traceables — each a spec id, its lens, and the filename it was declared in.
It is language-neutral: it knows the artifacts, the ids they declare, and the lens of each, not any target language.
Carrying each id, its lens, and its filename lets the traceables be grouped into spec sets — the configured groupings (a regex matched against the filename) a generator emits as separate symbol sets, by lens by default — which is how generators organize their output.
