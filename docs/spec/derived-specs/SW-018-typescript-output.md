...
The markers express the three relations of the taxonomy (ADR-0004): `traces` marks the code that **realizes** a spec — a value or function as `traces(ids, value)`, a class or method as the decorator `@traces(ids)`; `verifies(ids, run)` marks a test that **verifies** a spec; and `concerns`, in the same two forms as `traces` (`concerns(ids, value)` or `@concerns(ids)`), marks code that **concerns** a spec — coupled to it, depending on or constrained by it, without realizing or verifying it.
