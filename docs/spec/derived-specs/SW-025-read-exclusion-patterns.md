...
**Description**
The scan reads two arrays from `.clewrc.json` (ENT-002): `exclude` and `unexclude`, each a list of glob patterns.
...
**Rationale**
A convention file in the repository root is the established pattern for project configuration, and reading the patterns from `.clewrc.json` keeps them version-controlled with the code, the shared configuration contract (ADR-0005 D6).
...
Given `exclude` and `unexclude` arrays, both are read as their lists of patterns.
A configuration that omits both arrays yields empty lists.
