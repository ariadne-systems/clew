...
**Description**
The system shall define and enforce **traceability coverage** over the directly-anchorable specs: only eligible relations count (`realizes`, `verifies`; not `concerns`), an uncovered spec fails by default, and an uncovered spec is admitted only through a committed waiver carrying a reason.
...
Inverting the burden — covered unless waived — over a universe of specs that code can anchor directly keeps the waiver list a genuine, reviewed inventory of gaps rather than mechanical noise (ADR-0005 D4, D5).
