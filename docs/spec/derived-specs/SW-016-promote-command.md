...
Each argument names a draft to root the promotion at — a story (the common case, which carries its specs through its references) or a spec — and the keyword `all` promotes every pending draft.
At least one argument is required: invoked with none, the command is an error, never a finalize of whatever happens to be pending (CON-028).
