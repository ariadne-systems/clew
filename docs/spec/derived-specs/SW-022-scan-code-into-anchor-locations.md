...
Which paths the core reads is shaped by the exclusions applied before discovery — the built-in defaults (CON-016), and the project's configured `exclude` / `unexclude` layered on them (SW-024).
