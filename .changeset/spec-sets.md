...
A project can declare a `specSets` section — an ordered list of `{ name, pattern }`, where `pattern` is a regular expression over a spec's filename — and `ariadne spec` emits one symbol set per declared set instead of the default one-set-per-lens.
