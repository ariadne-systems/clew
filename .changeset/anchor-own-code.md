...
`core`, `cli`, `gen-typescript`, and `gen-java` now depend on `@ariadne-thread/trace`. The anchors are inert at runtime — `realizes` returns its value, the decorator is a no-op, `verifies` runs its tests — so behaviour is unchanged; deleting a spec now breaks compilation at every anchor that named it.
